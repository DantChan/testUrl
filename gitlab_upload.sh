...
unknown_os ()
{
  ...
  echo "You can find a list of supported OSes and distributions on our website: http://packages.gitlab.com/docs#os_distro_version"
  echo
...
finalize_yum_repo ()
{
  ...
  if [ "$?" != "0" ]; then
    ...
    echo "More information: http://fedoraproject.org/wiki/EPEL#How_can_I_use_these_extra_packages.3F"
    echo
...
main ()
{
  ...
  yum_repo_config_url="http://packages.gitlab.com/install/repositories/gitlab/gitlab-ce/config_file.repo?os=${os}&dist=${dist}&source=script"
